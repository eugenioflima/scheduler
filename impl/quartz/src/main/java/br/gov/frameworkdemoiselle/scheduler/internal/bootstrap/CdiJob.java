...
import br.gov.frameworkdemoiselle.scheduler.internal.bootstrap.ConcurrentQualifier;
...
public class CdiJob implements org.quartz.Job {
    ...
    private void startContexts() {
        if (!registered) {
        	...
        	RequestContext requestContext = Beans.getReference(RequestContext.class, new ConcurrentQualifier());
            SessionContext sessionContext = Beans.getReference(SessionContext.class);
            ViewContext viewContext = Beans.getReference(ViewContext.class, new ConcurrentQualifier());
